...
from signlang.data import SignVideoDataset, VideoRecord, class_names, gather_video_records, split_records
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a sign-language recognizer.")
    parser.add_argument("--data-dir", default="data/raw")
    parser.add_argument("--artifact-dir", default="artifacts/run")
    ...
    parser.add_argument(
        "--init-checkpoint",
        default=None,
        help="Initialize model weights from a checkpoint but start a fresh optimizer/scheduler state",
    )
...
def main() -> None:
    ...
    train_dataset = SignVideoDataset(train_records, class_to_idx, args.num_frames, args.image_size, training=True)
    val_dataset = SignVideoDataset(val_records, class_to_idx, args.num_frames, args.image_size, training=False)
    test_dataset = SignVideoDataset(test_records, class_to_idx, args.num_frames, args.image_size, training=False)
    ...
    print(
        ...
        f"{len(test_dataset)} test videos across {len(labels)} classes"
    )
...
def run_epoch(
    ...
    for clips, targets, _paths in progress:
...
def save_checkpoint(
    ...
    epoch: int,
    split_records_map: dict[str, list[VideoRecord]],
) -> None:
    checkpoint = {
        "epoch": epoch,
