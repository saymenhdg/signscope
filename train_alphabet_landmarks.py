...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an alphabet landmark recognizer.")
    parser.add_argument("--data-path", default="data/alphabet_landmarks_v2/records.json")
    parser.add_argument("--artifact-dir", default="artifacts/alphabet_landmarks_v2")
...
def main() -> None:
    ...
    records = load_landmark_records(args.data_path)
    if args.classes:
    ...
    if args.max_samples_per_class is not None:
        records = limit_per_class(records, args.max_samples_per_class)

    labels = class_names(records)
    class_to_idx = {label: index for index, label in enumerate(labels)}
    train_records, val_records, test_records = split_landmark_records(
        records=records,
        val_ratio=args.val_ratio,
        test_ratio=args.test_ratio,
        seed=args.seed,
    )
    input_dim = feature_dim(records)

    train_dataset = AlphabetLandmarkDataset(train_records, class_to_idx, training=True)
    val_dataset = AlphabetLandmarkDataset(val_records, class_to_idx, training=False)
    test_dataset = AlphabetLandmarkDataset(test_records, class_to_idx, training=False)
    class_weights = build_class_weights(train_records, class_to_idx)
    sampler = build_train_sampler(train_records) if args.balanced_sampler else None
    ...
    print(
        ...
        f"{len(test_dataset)} test landmark samples across {len(labels)} classes"
    )
    print(f"Trainable parameters: {count_trainable_parameters(model):,}")
    if args.balanced_sampler:
        print("Using class-balanced sampling for landmark training")
...
def limit_per_class(records: list[LandmarkRecord], max_samples_per_class: int) -> list[LandmarkRecord]:
    limited: list[LandmarkRecord] = []
...
def save_checkpoint(
    ...
    split_records_map: dict[str, list[LandmarkRecord]],
) -> None:
    checkpoint = {
