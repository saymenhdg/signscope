...
from signlang.inference import load_checkpoint
...
def main() -> None:
    ...
    if test_records:
        records = [VideoRecord(path=Path(item["path"]), label=item["label"]) for item in test_records]
    else:
        all_records = gather_video_records(args.data_dir)
