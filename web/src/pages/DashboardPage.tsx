        ...
        {QUICK_ACTIONS.map((action) => {
          ...
          return (
            <Link
              ...
              to={action.to}
              className={cn(
                'group relative overflow-hidden rounded-[30px] border border-outline-variant/10 p-7 shadow-lg transition-transform hover:-translate-y-1',
                action.accent,
              )}
            >
              <div className="mb-5 flex size-12 items-center justify-center rounded-2xl bg-black/10 backdrop-blur-xl">
              ...
              <ArrowRight className="mt-6 size-4 opacity-70 transition-transform group-hover:translate-x-1" />
            </Link>
          )
        })}
      </section>
      ...
      <div className="mt-8 grid gap-8 xl:grid-cols-[0.42fr_0.58fr]">
        <div className="space-y-8">
            ...
            <div className="mt-8 flex justify-center">
              <div className="relative flex size-48 items-center justify-center rounded-full border-[14px] border-surface-container-highest">
                <div
                  ...
                  style={{
                    background: `conic-gradient(#44e2cd 0deg ${(overview?.stats.daily_goal_percent ?? 0) * 3.6}deg, transparent 0deg)`,
                ...
                <div className="text-center">
                  <p className="font-headline text-5xl font-black text-on-surface">
                    {overview?.stats.daily_goal_percent ?? 0}%
                  </p>
          ...
          <Card className="rounded-[30px] border-outline-variant/12 bg-surface-container-low/90 p-8">
            ...
            <p className="mt-6 font-headline text-2xl font-bold text-on-surface">
              {overview?.insight.headline ?? 'Analyzing your recent activity'}
            </p>
            <p className="mt-3 text-sm leading-7 text-on-surface-variant">
              {overview?.insight.detail ?? 'Fetching the strongest next drill for your profile.'}
            </p>
        ...
        <div className="space-y-8">
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard label="Streak" value={overview ? `${overview.stats.practice_streak} days` : '...'} />
            <StatCard label="Live Accuracy" value={overview ? `${overview.stats.live_accuracy}%` : '...'} />
          ...
          <Card className="rounded-[30px] border-outline-variant/12 bg-surface-container-low/90 p-8">
            ...
            <div className="mt-8 space-y-4">
              {overview?.recent_translations.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col gap-3 rounded-[24px] border border-outline-variant/10 bg-surface-container p-5 sm:flex-row sm:items-center"
                >
                ...
                <div className="rounded-[24px] border border-outline-variant/10 bg-surface-container p-5 text-sm text-on-surface-variant">
                  Loading recent transcript history...
                </div>
              )}
            </div>
          </Card>
