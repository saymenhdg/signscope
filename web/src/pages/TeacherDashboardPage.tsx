...
export function TeacherDashboardPage() {
  ...
  const displayName = data?.profile_card?.display_name?.split(' ')[0] ?? 'there'
      ...
      <div>
        ...
        <div className="mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-on-surface-variant">Here&apos;s your teaching overview for today.</p>
          </div>
          <div className="relative" ref={notificationsRef}>
            <button
              type="button"
              onClick={() => setNotificationsOpen((current) => !current)}
              className="relative flex size-14 items-center justify-center rounded-full border border-outline-variant/20 bg-surface-container text-primary shadow-lg shadow-primary/5"
              aria-label="Notifications"
            >
              <Bell className="size-5" />
              {(notifications?.unread_count ?? 0) > 0 ? (
                <>
                  <span className="absolute right-4 top-4 size-2 rounded-full bg-error" />
                  <span className="absolute -right-1 -top-1 flex min-w-6 items-center justify-center rounded-full bg-primary px-1.5 py-1 text-[10px] font-bold text-background">
            ...
            {notificationsOpen ? (
              <div className="absolute right-0 top-16 z-20 w-[22rem] rounded-3xl border border-outline-variant/20 bg-surface-container p-4 shadow-2xl backdrop-blur-xl">
                <div className="mb-4 flex items-center justify-between gap-3">
                  <div>
                    <p className="font-headline text-xl font-bold text-on-surface">Notifications</p>
                    <p className="text-sm text-on-surface-variant">{notifications?.unread_count ?? 0} active alerts</p>
                  </div>
                </div>
                  ...
                  {notifications?.items.length ? (
                    notifications.items.map((item) => (
                      <Link
                        ...
                        className="block rounded-2xl border border-outline-variant/10 bg-surface-container-low p-4 transition-colors hover:bg-surface-container-high"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <p className="font-semibold text-on-surface">{item.title}</p>
                            <p className="mt-1 line-clamp-2 text-sm text-on-surface-variant">{item.detail}</p>
                            <p className="mt-2 text-xs text-on-surface-variant">{new Date(item.created_at).toLocaleString()}</p>
                          </div>
                    ...
                    <div className="rounded-2xl border border-dashed border-outline-variant/20 bg-surface-container-low p-5 text-center">
                      <p className="font-headline text-lg font-bold text-on-surface">You&apos;re all caught up</p>
                      <p className="mt-2 text-sm text-on-surface-variant">New booking requests and unread messages will appear here.</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-6">
            <section>
              <div className="mb-4 flex items-center justify-between gap-4">
                <h2 className="font-headline text-3xl font-bold text-on-surface">Today&apos;s Schedule</h2>
                <Link to="/app/teacher/settings" className="text-sm font-semibold text-primary/70 hover:text-primary">
                  Profile Setup
                </Link>
              </div>
            ...
            <section>
              <div className="mb-4 flex items-center justify-between gap-4">
                <h2 className="font-headline text-3xl font-bold text-on-surface">Weekly Earnings</h2>
                {data && (
              ...
              <Card className="rounded-3xl p-6">
                {isLoading ? (
                  ...
                  <div>
                    <p className="font-headline text-5xl font-bold tracking-tight text-on-surface">{weeklyEarnings}</p>
                    <p className="mt-2 text-sm text-on-surface-variant">Total this week</p>
                        ...
                        <div key={index} className="flex flex-1 flex-col items-center gap-3">
                          <div
                            className={cn(
                              ...
                              bar.active ? 'bg-primary shadow-lg shadow-primary/20' : 'bg-surface-container-highest',
                            )}
          ...
          <aside className="space-y-6">
            ...
            <Card className="rounded-3xl p-5">
              <div className="mb-5 flex items-center justify-between gap-4">
                <CardTitle className="text-xl">Student Requests</CardTitle>
                <div className="flex size-7 items-center justify-center rounded-full bg-tertiary/15 text-xs font-bold text-tertiary">
                  {pendingCount}
                </div>
              </div>
...
function ScheduleRow({
  ...
  return (
    <div
      className={cn(
        'flex flex-col gap-4 rounded-2xl px-4 py-5 sm:flex-row sm:items-center',
        isPrimary ? 'border border-outline-variant/10 bg-surface-container' : 'bg-transparent hover:bg-surface-container',
      )}
    >
      <div className={cn('hidden w-1 self-stretch rounded-full sm:block', isPrimary ? 'bg-secondary' : 'bg-transparent')} />
      <div className="w-20 shrink-0">
        <p className="font-headline text-3xl font-bold tracking-tight text-on-surface">{time}</p>
        <p className="mt-1 text-xs font-bold uppercase tracking-[0.18em] text-on-surface-variant">{meridiem}</p>
      </div>

      <div className="min-w-0 flex-1">
        <p className="truncate font-headline text-xl font-bold text-on-surface">
      ...
      <div className="flex flex-wrap gap-3">
        {booking.status === 'pending' ? (
          ...
          <Badge className="border-secondary/10 bg-secondary/10 px-5 py-2 text-sm font-semibold capitalize text-secondary">
            {booking.status}
          </Badge>
        )}
      </div>
  ...
  return (
    <div className="rounded-2xl border border-outline-variant/10 bg-surface-container p-4">
      <div className="flex items-start gap-3">
        <div className="flex size-11 shrink-0 items-center justify-center rounded-full bg-primary/15 font-headline text-sm font-black text-primary">
          {initials}
        </div>
        <div className="min-w-0 flex-1">
          <p className="truncate font-semibold text-on-surface">{booking.student_name}</p>
      ...
      <div className="mt-4 flex gap-2">
        {booking.status === 'pending' ? (
          ...
          <div className="flex w-full items-center justify-center rounded-full border border-outline-variant/20 bg-surface-container-high px-4 py-2 text-sm font-semibold capitalize text-on-surface-variant">
            {booking.status}
          </div>
        )}
      </div>
...
function MiniStatCard({
  ...
  return (
    <Card className="rounded-3xl p-5">
      ...
      <p className="mt-6 font-headline text-5xl font-bold tracking-tight text-on-surface">{value}</p>
