      ...
      <div className="grid gap-8 xl:grid-cols-[0.95fr_1.25fr]">
        <Card className="rounded-3xl p-5">
          <div className="mb-5 flex items-center justify-between gap-4">
            <div>
              <h2 className="font-headline text-2xl font-bold text-on-surface">Inbox</h2>
              <p className="text-sm text-on-surface-variant">Threads started from teacher bookings.</p>
            </div>
            <Badge>{inbox?.unread_count ?? 0} unread</Badge>
          </div>

          <div className="mb-5 flex items-center gap-3 rounded-xl border border-outline-variant/20 bg-surface-container px-4 py-3 text-sm text-on-surface-variant">
                ...
                >
                  <div className="flex items-start gap-3">
                    {thread.student_avatar_url ? (
                      ...
                      <div className="flex size-11 items-center justify-center rounded-full bg-primary/15 font-headline text-sm font-bold text-primary">
                        {thread.student_name
                          .split(' ')
                          .map((part) => part[0])
                          .join('')
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-3">
                        <p className="truncate font-semibold text-on-surface">{thread.student_name}</p>
                        <span className="text-xs text-on-surface-variant">{new Date(thread.last_message_at).toLocaleDateString()}</span>
                      </div>
                      <p className="mt-1 line-clamp-2 text-sm text-on-surface-variant">{thread.last_message_preview}</p>
                      <div className="mt-3 flex items-center gap-2">
                        <Badge>{thread.booking_count} bookings</Badge>
                        {thread.unread_count > 0 ? (
                          <Badge className="border-secondary/10 bg-secondary/10 text-secondary">{thread.unread_count} new</Badge>
                        ) : null}
                      </div>
                    </div>
                  </div>
        ...
        <Card className="flex min-h-[42rem] flex-col rounded-3xl p-0">
          {activeThread ? (
            <>
              <div className="border-b border-outline-variant/20 px-6 py-5">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <h2 className="font-headline text-2xl font-bold text-on-surface">{activeThread.student_name}</h2>
                    <p className="mt-1 text-sm text-on-surface-variant">{activeThread.student_email}</p>
                  </div>
                  <Badge>{activeThread.booking_count} bookings</Badge>
                </div>
              </div>
                    ...
                    >
                      <div
                        className={cn(
                          ...
                          message.is_own
                            ? 'bg-gradient-to-br from-primary to-primary-container text-background'
                            : 'border border-outline-variant/10 bg-surface-container text-on-surface',
                        )}
                      >
                        <p className="text-sm leading-7">{message.body}</p>
                        <p className={cn('mt-2 text-[11px]', message.is_own ? 'opacity-70' : 'text-on-surface-variant')}>
              ...
              <form onSubmit={handleSend} className="border-t border-outline-variant/20 p-5">
                <div className="flex items-end gap-3">
                  <textarea
                    value={draft}
                    onChange={(event) => setDraft(event.target.value)}
                    className="min-h-24 flex-1 rounded-2xl border border-outline-variant/25 bg-surface-container px-4 py-3 text-sm text-on-surface outline-none transition-all placeholder:text-on-surface-variant/70 focus:border-secondary/60"
...
function EmptyMessagesState({ compact = false }: { compact?: boolean }) {
  return (
    <div className={cn('rounded-2xl border border-dashed border-outline-variant/20 bg-surface-container text-center', compact ? 'p-5' : 'p-8')}>
      ...
      <p className="mt-4 font-headline text-xl font-bold text-on-surface">No conversations yet</p>
